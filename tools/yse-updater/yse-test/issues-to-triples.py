...
yse_skos = Graph().parse('yse-test-skos.ttl', format='turtle')
...
def headingToProperty(block):
    ...
    'Ehdotettu termi pohjoissaameksi': 'prefLabel@se',
    'Ehdotetut temaattiset ryhmät': 'member',
    'Ehdotettu yläkäsite YSOssa (LT)': 'broadMatch',
    'Ehdotettu yläkäsite (LT)': 'broadMatch',
    'Tarkoitusta täsmentävä selite': 'note',
    'Vaihtoehtoiset termit': 'altLabel',
    'Vaihtoehtoiset termit suomeksi': 'altLabel@fi',
    'Vaihtoehtoiset termit ruotsiksi': 'altLabel@sv',
    'Vaihtoehtoiset termit englanniksi': 'altLabel@en',
    'Vaihtoehtoiset termit pohjoissaameksi': 'altLabel@se',
    'Alakäsitteet (RT)': 'narrowMatch',  # typo, but in use
    'Alakäsitteet (ST)': 'narrowMatch',
    'Assosiatiiviset (RT)': 'relatedMatch',
    'Assosiatiiviset käsitteet (RT)': 'relatedMatch'
    }
...
def addPropertyValueTriples(prop, value, uri):
    if prop == 'type':
        ...
        if value == 'SLM':
            newtriples.add( (rdflib.term.URIRef(uri), RDF.type, rdflib.term.URIRef(ysemeta + 'GenreConcept')) )
    elif prop == 'member': # group
        for val in value.split('; ' if '; ' in value else ', '):
            group_number = val[1:3]
    ...
    elif '](http://' not in value: # prefLabel and altLabel
        if '@' in prop:
            prop, val_lang = prop.split('@')
            if '; ' in value: # widget 2.0 altLabels
                for val in value.split('; '):
                    newtriples.add( (rdflib.term.URIRef(uri), rdflib.term.URIRef(skos + prop), rdflib.term.Literal(val, lang=val_lang)) )
                return
        else:
            val_lang = 'fi'
        newtriples.add( (rdflib.term.URIRef(uri), rdflib.term.URIRef(skos + prop), rdflib.term.Literal(value, lang=val_lang)) )
    else: #broadMatch, narrowMatch and relatedMatch
        for val in value.split('](')[1:]:
            value_uri = val.split(')', 1)[0].strip()
...
def issueToTriple(issue):
    uri = yso + 'Y' + str(issue.number + 100000)
    ...
    newtriples.add( (rdflib.term.URIRef(uri), foaf.homepage, rdflib.term.URIRef('https://github.com/NatLibFi/YSE-test/issues/' + str(issue.number))) )
...
for issue in new_issues:
    uri = 'http://finto.fi/yse/fi/page/Y' + str(issue.number + 100000)
    ...
    if 'Termiehdotus Fintossa' not in newbody:
        newbody = newbody + '\n\n**Termiehdotus Fintossa:** [' + issue.title + '](' + uri + ')'
    issueToTriple(issue)
    if edit_issues == True:
        issue.edit(body=newbody);
        issue.remove_from_labels(accept_lab)

newtriples.serialize(destination='yse-test-new.ttl', format='turtle')
yse_skos.parse('yse-test-new.ttl', format='turtle')
yse_skos.serialize(destination='yse-test.ttl', format='turtle')
