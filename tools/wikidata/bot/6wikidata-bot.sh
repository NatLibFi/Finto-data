...
./6dots.sh 5
echo "The dates on Wikidata side"
...
# Retrieving data from the database and generating reports
...
./6dots.sh 3
echo "On deprekoitu YSOssa mutte ei Wikidatassa"
echo "Luodaan raportti"
...
./6dots.sh 3
echo "On Wikidatassa mutta ei ysossa"
echo "Luodaan raportti"
...
WHERE w.wd_rank != 'DeprecatedRank'
  ...
  AND NOT EXISTS (SELECT 1 FROM yso_wd_links yl WHERE yl.yso_concept_uri = y.yso_concept_uri AND yl.wd_entity_uri = w.wd_entity_uri);
