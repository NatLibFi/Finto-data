...
# Slightly modified timestamper to take into account quirks in Metatietosanasto.
# Generates dct:created and dct:modified timestamps (xsd:date) for all subjects
# (instead of only concepts) that start with the MTS PURI context. Note that
# the terms/names used in comments/variables may be misleading as they were not
# updated in the process. Additionally, requires Python 3.8+.
...
if sys.version_info < (3,8,0):
    print("%s requires Python 3.8.0 or newer to work!" % sys.argv[0], file=sys.stderr)
    sys.exit(1)
...
def concept_hash(concept):
    ...
    sorted_nt = b''.join(sorted([x.encode('utf-8') for x in nt.splitlines(True)]))
    md5 = hashlib.md5()
    md5.update(sorted_nt)
    return md5.hexdigest()

new_timestamps = {}
for concept in g.subjects(None, None):
    if not concept.startswith("http://urn.fi/URN:NBN:fi:au:mts:m"):
        continue
...
sys.stdout.write(g.serialize(destination=None, format='turtle'))
