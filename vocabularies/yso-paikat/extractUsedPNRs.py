...
def readCommandLineArguments():
    ...
    parser.add_argument("-i", "--input", nargs='?', type=argparse.FileType('r'), help="Input file location for PNR data, e.g., pnr.csv. Expects the following (comma separated) column order: 0: 'paikkaID', 1: 'paikkatyyppi', 2: 'WGS84_N', 3: 'WGS84_E", default=sys.stdin)
...
def main():
    ...
    for row in reader:
        if select_all or row[0] in selected:
            ...
            print('pnr:' + row[0] + ' ysometa:mmlPlaceType ysometa:MPT' + row[1] + ' .')
